//
// bmp2spbas
// 16色のビットマップファイルからスプライト用のBASICソースを生成するツール
// Copyright 2023 CHIKUWA TEIKOKU
//
// bmp2spbas <in:BITMAP FILE NAME> <out:BASIC FILE NAME> [START ROW NUMBER] [INCLEMENTAL ROW NUMBER]
//
//   <in:BITMAP FILE NAME>		入力するビットマップファイル名(256x192までの8の倍数のサイズ)
//   <out:BASIC FILE NAME>		出力するBASICソースファイル名
//   [START ROW NUMBER]			開始行番号
//   [INCLEMENTAL ROW NUMBER]	行番号の増分
//
// ex.
//   bmp2spbas example.bmp example_sprite.bas 10000 10
//
...
int main(int argc, char* argv[])
{
	...
	ret_code++;
	if (argc < 3) {
		fprintf(stderr, "USAGE: bmp2spbas <in:BITMAP FILE NAME> <out:BASIC FILE NAME> [START ROW NUMBER] [INCLEMENTAL ROW NUMBER]\n");
		goto LABEL_END;
	}
	...
	for (int y = 0; y < info_header.height / 16; y++)
	{
		for (int x = 0; x < info_header.width / 16; x++)
		{
			...
			for (int c = 1; c < 16; c++)
			{
				...
				fprintf(fp_ptn, "%05u 'SPRITE: NO=%d COLOR=%d\n", row_num, sprite_no, c);
				row_num += inc_row;
				...
				{
					// パターンデータを2進数で出力する
					fprintf(fp_ptn, "%05u DATA ", row_num);
					for (int j = 0; j < 8; j++)
					{
